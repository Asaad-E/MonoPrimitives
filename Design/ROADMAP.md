...
- **NuGet: published.** `MonoPrimitives 0.2.0` is live on nuget.org (`dotnet add package MonoPrimitives`), via `.github/workflows/publish.yml` (GitHub Release → NuGet Trusted Publishing, OIDC, no stored API key).
