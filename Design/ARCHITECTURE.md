...
| `Camera2D.cs` | Transform matrix, screen↔world, bounds/padding, smooth-follow/zoom. Three-tier update: `Update` (no input, just decays shake/settles easing — for a camera you drive yourself), `Update(in CameraInput2D, float)` (applies explicit input from anywhere), `UpdateWithInput` (reads real WASD/drag/wheel via `PrimitiveInput` — a prototyping convenience, not baked into `Update`). Same three-tier shape as `Camera3D`. Trauma-based screen shake (`AddTrauma`, baked into `GetTransformMatrix`) — see DECISIONS.md. Optionally takes a `ViewportAdapter2D` at construction (MonoGame.Extended's `OrthographicCamera(ViewportAdapter)` shape) — when set, `ScreenToWorld`/`WorldToScreen`/`GetVisibleWorldBounds`/the mouse-drag part of `ReadDefaultInput` all account for it automatically instead of assuming raw device pixels. |
