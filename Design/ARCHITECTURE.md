...
| File | Purpose |
|---|---|
| `Primitives2D.cs` | `PrimitiveBatch` — all shape drawing (Fill/Border/Draw per shape, rounded-corner and gradient variants), outline/fillet engine, points/lines/splines/`DrawArrow`/`DrawGrid` (`showMajorLines: bool = true`, `lineThickness: float = 1f` — minor thickness, major is always +1)/`DrawAxis` (`thickness: float = 1f`)/`Fill*Shadow` (soft drop shadows, no shader — see DECISIONS.md). Large; grep, don't read linearly. |
