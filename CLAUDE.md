...
**Before doing anything**, read **[`Design/README.md`](Design/README.md)** — it links to 6 short docs (project brief, architecture map, code style, doc-comment style, decisions, roadmap) covering current state. Together under 2,500 words; read all six before starting real work, not just the one that seems most relevant.
...
Don't read the historical logs under `Design/archive/` unless specifically digging into why a past bug fix happened — they're large and superseded by the docs above for anything about *current* state.
