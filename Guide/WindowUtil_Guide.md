...
`WindowUtil` (namespace `MonoPrimitives`, file [`src/Core/WindowUtil.cs`](../src/Core/WindowUtil.cs)) covers window/monitor/clipboard operations `GameWindow` and `GraphicsDeviceManager` don't expose at all: minimize/maximize/restore/show/hide, opacity, resize constraints, a title-bar/taskbar icon, per-monitor position/size/refresh rate/DPI scale, and system clipboard text — plus a captured-cursor delta helper for mouse-look input.
...
- **`SetWindowMinSize`/`SetWindowMaxSize` are a hint to the OS/window manager, not a hard guarantee.** Usually honored reliably (including on a non-resizable window), but observed once not to clamp a same-session resize after several other window-state changes, with no reliable repro found despite trying — most likely OS-level timing, not a bug in these methods. Use it for "usually keeps the window from getting uncomfortably small," not for correctness-critical layout.
