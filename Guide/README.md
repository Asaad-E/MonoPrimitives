...
Reference docs for every public class in **MonoPrimitives**: immediate-mode 2D/3D primitive drawing for MonoGame, plus camera, input, easing, color, noise, and collision — the stuff every prototype ends up needing anyway.

Each guide stands on its own: what the class does, a runnable snippet, a full method reference, and whatever gotchas are worth knowing before you hit them. No need to read in order — jump to whatever you're building.

## Before you start

These assume you already have a MonoGame project with a working `Game` class (`Initialize`/`LoadContent`/`Update`/`Draw`). They teach MonoPrimitives, not MonoGame. Starting from scratch:

```bash
dotnet add package MonoPrimitives
```

or grab the starter template (letterboxed virtual resolution, MSAA, input, all wired together):
...
The root [`README.md`](../README.md) has a two-minute quick-start if you just want to see the shape of the API first.

## If you're brand new here

Everything else exists to support drawing, so start there:
...
Then add a camera ([`Camera2D`](Camera2D_Guide.md) / [`Camera3D`](Camera3D_Guide.md)) and input ([`PrimitiveInput`](PrimitiveInput_Guide.md)). Those three cover most of what a small game or simulation needs — everything else is there for when you need it.
...
- [`DebugFont5x7`](DebugFont5x7_Guide.md) — a built-in bitmap debug font, 2D and 3D (billboarded), with word-wrap.

**Camera & viewport**
- [`Camera2D & Viewport`](Camera2D_Guide.md) — pan/rotate/zoom, bounds, follow, shake, and the `ViewportAdapter2D` family for letterboxing/scaling. Worth reading even for 3D-only projects — 3D shares the same adapters.
...
- [`Noise`](Noise_Guide.md) — seedable Perlin noise, fBm, ridge, and turbulence, for terrain and organic variation.
...
- [`VectorExtensions`](Vector2Extensions_Guide.md) — angle/rotation/approach/clamp helpers on MonoGame's own `Vector2`, plus `Vector3Extensions` in the same guide.
- [`RectangleF`](RectangleF_Guide.md) — a float-precision counterpart to MonoGame's integer-only `Rectangle`.
- [`UnitCircleLut`](UnitCircleLut_Guide.md) / [`TrigLut`](TrigLut_Guide.md) — trig-free lookup tables, the same ones the shape batches use internally, for your own curved geometry.
- [`RingBuffer`](RingBuffer_Guide.md) — a generic fixed-capacity ring buffer, the same thing `Trail2D`/`Trail3D`/`FpsCounter` already use privately, exposed for your own history/log/sample-window need.
- [`ObjectPool`](ObjectPool_Guide.md) — a generic object pool for anything spawned/discarded often enough (bullets, particles, agents) to want reuse over reallocation.
...
Check [`Design/DECISIONS.md`](../Design/DECISIONS.md) — that's where the reasoning behind a non-obvious behavior lives (a clamp, a naming choice, a bug that got fixed). [`Design/ROADMAP.md`](../Design/ROADMAP.md) lists what's deliberately out of scope, in case what you're looking for was considered and skipped on purpose.
